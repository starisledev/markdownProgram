...
(function (global) {
  ...
  function openDoc(id) {
    ...
    var stDoc = $('stDoc');
    if (stDoc) { stDoc.textContent = f.name; stDoc.title = f.name; }
  ...
  function runAct(act) {
    switch (act) {
      ...
      case 'toggleToolbar': toggleToolbar(); break;
  ...
  function showAbout() {
    modal('关于砚屿', '<div style="line-height:1.9">' +
      '<b style="font-size:15px">砚屿 1.3</b> — 所见即所得 Markdown 编辑器。<br>' +
      '文档保存在本地，不联网、不上传。<br>' +
  ...
  function syncToolbar() {
    var tb = $('toolbar');
    if (!tb || sourceMode) return;
    var st = Editor.state();
    $$('.tool[data-cmd]', tb).forEach(function (b) {
      var c = b.getAttribute('data-cmd');
      var on = (c === 'bold' && st.bold) || (c === 'italic' && st.italic) ||
               (c === 'strike' && st.del) || (c === 'mark' && st.mark) ||
               (c === 'code' && st.code);
      b.classList.toggle('on', !!on);
    });
    var selb = $('tbBlock');
    if (selb) selb.value = st.block || 'p';
  }

  /** 状态栏光标位置：行 = 所在顶层块的序号，列 = 块内文本偏移 */
  function updateCursorPos() {
    var el = $('stPos');
    if (!el) return;
    if (sourceMode) {
      var idx = sourceEl.selectionStart || 0;
      var before = sourceEl.value.slice(0, idx);
      el.textContent = '第 ' + before.split('\n').length + ' 行，第 ' + (idx - before.lastIndexOf('\n')) + ' 列';
      return;
    }
    var s = window.getSelection();
    if (!s.rangeCount || !editorEl.contains(s.getRangeAt(0).commonAncestorContainer)) {
      el.textContent = '';
      return;
    }
    var r = s.getRangeAt(0);
    var block = r.startContainer;
    var line = 0;
    while (block && block.parentNode !== editorEl) block = block.parentNode;
    if (block === editorEl) {
      line = r.startOffset + 1; // 光标位于块之间
      el.textContent = '第 ' + line + ' 行，第 1 列';
      return;
    }
    if (block && block.parentNode === editorEl) {
      line = Array.prototype.indexOf.call(editorEl.children, block) + 1;
    } else {
      el.textContent = '';
      return;
    }
    ...
    el.textContent = '第 ' + line + ' 行，第 ' + col + ' 列';
  }
  ...
  function toggleToolbar() {
    var tb = $('toolbar');
    if (!tb) return false;
    var on = tb.classList.toggle('expanded');
    Store.setSetting('toolbar', on);
    return on;
  }

  /* 绑定工具栏：按钮走统一命令分发，段落下拉走块类型转换 */
  function bindToolbar() {
    ...
    tb.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      runAct(btn.getAttribute('data-cmd'));
    });
    var selb = $('tbBlock');
    if (selb) {
      selb.addEventListener('change', function () {
        execAction(selb.value);
      });
    }
  }
  ...
  function init() {
    ...
    if (Store.getSetting('toolbar', false)) { var _tb = $('toolbar'); if (_tb) _tb.classList.add('expanded'); }
    ...
    // 打开上次文档；Ctrl+N 新窗口（?new=1）直接新建空白文档
    var startNew = false;
    ...
    editorEl.addEventListener('click', function () { syncToolbar(); updateCursorPos(); });
    editorEl.addEventListener('keyup', function () { updateCursorPos(); });

...
(function (global) {
  ...
  var WELCOME = [
    ...
    '## 快捷键',
    '',
    '- `Ctrl + B` 加粗　`Ctrl + I` 斜体　`Ctrl + U` 高亮　`Ctrl + E` 行内代码',
    '- `Ctrl + D` 删除线　`Ctrl + K` 插入链接',
    '- `Ctrl + F` 查找替换　`Ctrl + T` 切换主题',
    '- `Ctrl + 1` 文件树　`Ctrl + 2` 大纲　`F11` 全屏',
    '- `Ctrl + N` 新建窗口　`Ctrl + O` 打开文件',
    '- `Ctrl + S` 导出 Markdown　`Ctrl + P` 打印 / 导出 PDF',
    ...
    '- [ ] 待办：写你自己的第一篇文档',
    '',
    '数学之外的常见语法基本都覆盖了 —— 试试「文件 → 新建文档」，或者直接把 `.md` 文件拖进窗口。',
    '',
    '---',
    '',
    '*文档自动保存在浏览器本地，不会上传。*'

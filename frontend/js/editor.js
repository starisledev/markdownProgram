...
(function (global) {
  ...
  function splitAtCaret(container) {
    var s = sel();
    var r = s.getRangeAt(0);
    // 关键修复：光标前没有文本内容（空段落 / 仅 <br> / 行首）时不抽取任何节点，
    // 避免把段落里唯一的 <br> 抽走 —— 否则连续回车永远停在同一条线（“回车不换行”）
    try {
      var preR = r.cloneRange();
      preR.selectNodeContents(container);
      preR.setEnd(r.startContainer, r.startOffset);
      if (!preR.toString()) {
        return doc().createDocumentFragment(); // 空 fragment：源段落保留其 <br>
      }
    } catch (e) { /* 区间异常则按原路径继续 */ }
    try {
      var rg = r.cloneRange();
